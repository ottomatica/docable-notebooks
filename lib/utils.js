...
let { logger, DOCKER_IMAGE, config, configPath, targets, targetsPath, dockerImages, secretsPass } = env.vars();
...
async function* tree(notebookDir, dir) {
    for await (const d of await fs.promises.opendir(dir)) {
        ...
        if (d.isDirectory()) {
            ...
            let normalizeNotebookDir = notebookDir.replace(/[/]/g, path.sep) 
            let name = entry.replace(normalizeNotebookDir,"");
            // console.log(notebookDir, entry, name);
            let link = `/notebooks/${notebookSlug.notebook2slug(name)}`;
...
async function prepareEnvironment(session, provider, isHosted, name, cwd)
{
    ...
    if (provider == 'local') {
        ...
        if (provider == 'docker') {
            ...
            await createContainer(session, containerName, containerTimeout);
        }
...
module.exports = {
    readNotebook, getNotebookList, getNotebookTree, _get_notebook,
