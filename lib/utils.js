...
let {logger, DOCKER_IMAGE, config, secretsPass} = env.vars();
...
async function notebookRender(md, ir) {
    ...
    try{
        ...
        $('pre').each(function (index, elem) {
            ...
            console.log( $(elem).hasClass("language-svg") )
...
function getSecrets() {
    ...
    return secrets.map(s => {
        return {
            slug: s.slug,
            secret: decryptWithKey(s.secret, secretsPass)
        }
    });
}

function setSecret(slug, secret) {
    const secrets = new Set(config.get('secrets'));
    secrets.add({
        slug: slug,
        secret: encryptWithKey(secret, secretsPass)
    });
    config.set('secrets', Array.from(secrets));
}
...
module.exports = {notebookRender, renderCell, getExamples, getNotebook, refreshEnv, createContainer, resetContainerTimeout, encryptWithKey, decryptWithKey, _getSecrets: getSecrets, setSecret};
