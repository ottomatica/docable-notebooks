...
async function* tree(notebookDir, dir, endpoint = 'notebooks') {
    for await (const d of await fs.promises.opendir(dir)) {
        ...
        if (d.isDirectory()) {
            ...
            // TODO: temp fix...?
            if (notebookDir.startsWith('\\') || notebookDir.startsWith('/')) 
            ...
            // console.log(notebookDir, entry, normalizeNotebookDir, name);
            let link = `/${endpoint}/${notebookSlug.notebook2slug(name)}`;
