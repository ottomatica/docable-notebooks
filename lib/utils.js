...
async function getNotebookTree(notebookDir) {
    let notebooks = [];
    for await (const p of tree(notebookDir)) {
...
async function* tree(dir) {
    for await (const d of await fs.promises.opendir(dir)) {
        ...
        if (d.isDirectory()) 
        {
            yield* {name: d.name, children: await tree(entry) };
        }
        else if (d.isFile()) yield entry;
    }
}
...
module.exports = {notebookRender, renderCell, getExamples, getNotebook, getNotebookList,
    refreshEnv, createContainer, resetContainerTimeout, 
    encryptWithKey, decryptWithKey, getVariables, setVariable,
