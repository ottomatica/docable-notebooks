...
exports.get_notebook = async function (req, res) {
    ...
    try {
        const notebook = await notebookManager._get_notebook(req, res, path.join('notebooks', repoPath), notebookMDPath, setCurrentEnvironment);
        ...
        let notebook_tree = await utils.getNotebookTree(path.dirname(path.resolve(repoDir, repoPath)), path.join('/notebooks/', path.dirname(req.params[0])));
...
function setCurrentEnvironment(session, notebookUrl) {
    const targets = session.notebooks[notebookUrl].setup.targets;
    let currentEnvironment = session.notebooks[notebookUrl].currentEnvironment
    if (!currentEnvironment) {
        const availableEnvironments = envManager.getAvailableEnvironments(targets, session, notebookUrl);
        session.notebooks[notebookUrl].availableEnvironments = availableEnvironments;

        session.notebooks[notebookUrl].currentEnvironment = availableEnvironments[0];
...
exports.get_target_status = async function (req, res) {
    ...
    if (req.session.notebooks[notebookUrl]) {
        ...
        res.status(404);
        res.send(`Notebook ${notebookUrl} not loaded!`);
    }
}
...
exports.get_github_imports = async function (req, res) {
    ...
    _get_notebook(req, res, notebookDir, notebookName, notebookRepoPath, setCurrentEnvironment);
}
