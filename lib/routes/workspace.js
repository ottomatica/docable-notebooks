...
async function _get_notebook(req, res, notebook_dir, slug, cwd) {
    ...
    try {
        ...
        let name = utils.slug2notebook(slug);

        logger.info(`Finding notebook: ${notebook_dir}/${name}`);
        const nb = await utils.getNotebook(slug, notebook_dir);
        ...
        const { html, IR, md } = await notebookRender.notebookRender(path.dirname(name), nb);
...
exports.variables = async function(req, res) {
    ...
    res.render('variables', { variables: utils.getVariables().map(v => ({ ...v, value: v.isSecret ? undefined : v.value}))});

}

exports.setVariable = async function(req, res) {

    utils.setVariable(req.body.slug, req.body.value, req.body.isSecret ? true : false, req.body.edit);
...
exports.targets = async function (req, res) {
    ...
    res.render('targets', { targets: utils.getTargets().map(t => ({ ...t, sshKey: undefined, sshKeyPath: undefined })) });
}
