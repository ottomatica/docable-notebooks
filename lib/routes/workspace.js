...
exports.get_notebook = async function (req, res) {
    ...
    try {
        ...
        if (req.session.notebooks[notebookUrl].currentEnvironment.type === 'local' &&
                req.session.notebooks[notebookUrl].currentEnvironment.cwd)
            cwd = path.resolve(req.session.notebooks[notebookUrl].currentEnvironment.cwd, req.session.notebooks[notebookUrl].docDir);
        else
            cwd = notebook_dir;

        await envManager.prepareEnvironment(req.session, req.session.notebooks[notebookUrl].currentEnvironment, cwd);
...
exports.set_environment = async function (req, res) {
    ...
    if (newEnvironment) {
        ...
        await envManager.prepareEnvironment(req.session, newEnvironment);
