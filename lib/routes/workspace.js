...
async function _get_notebook(req, res, notebook_dir, slug, cwd) {
    ...
    try {
        ...
        const { html, IR, md } = await utils.notebookRender(path.dirname(name), nb);
...
exports.variables = async function(req, res) {
    res.render('variables', { variables: utils.getVariables()});
}

exports.setVariable = async function(req, res) {

    utils.setVariable(req.body.slug, req.body.value, req.body.isSecret ? true : false);
...
exports.addTarget = async function (req, res) {
    utils.addTarget({ ...req.body }, req.body.force);
