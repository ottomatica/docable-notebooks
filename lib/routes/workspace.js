...
let {logger, configPath, config, githubImportsDir} = env.vars();
...
exports.get_notebook = async function (req, res) {
    ...
    try {
        ...
        if( provider == "local") {
            if (session.notebooks.setup.local.cwd)
                cwd = path.resolve(session.notebooks.setup.local.cwd, session.notebooks.docDir);
            else
                cwd = notebook_dir;
        }

        await envManager.prepareEnvironment(session, provider, cwd);

        res.render('notebook', { ...notebook, isHosted, breadcrumb: notebookMDPath.split('/') });
    }
    catch (err) {
        logger.warn(err);
        res.status(404);
...
exports.set_environment = async function(req, res) {
    ...
    let terminalCommand;
    let terminalCommandArgs;

    let setupStanza;

    // setting setupStanza and terminal command
    if( id.match(/docker:.+/))
    {
        let image = id.match(/docker:(.+)/)[1];
        setupStanza = yaml.safeLoad(dockerStanzaTemplate('newname-uuidrandom', image)).setup;

        terminalCommand = process.platform === 'win32' ? `docker.exe` : 'docker';
        terminalCommandArgs = ['exec', '-i', '-t', 'newname-uuidrandom',
            'bash', '-c', `echo \"export PS1='\\[\\e[31m\\][\\[\\e[m\\]\\[\\e[38;5;172m\\]\\u\\[\\e[m\\]@\\[\\e[38;5;153m\\]\\h\\[\\e[m\\] \\[\\e[38;5;214m\\]\\W\\[\\e[m\\]\\[\\e[31m\\]]\\[\\e[m\\]\\$ '\" >> ~/.bashrc && bash`];
    }
    else if( id == "local" )
    {
        setupStanza = yaml.safeLoad(localhostTemplate()).setup;

        terminalCommand = process.platform === 'win32' ? `powershell.exe` : 'bash';
    }
    else if( id.match(/ssh:.+/))
    {
        ...
        terminalCommand = process.platform === 'win32' ? `ssh.exe` : 'ssh';
        terminalCommandArgs = ['-i', target.sshKeyPath, `${target.username}@${target.ip}`, '-p', target.port == '' ? '22' : target.port];
    }

    req.session.notebooks.setup = setupStanza;
    req.session.notebooks.terminalCommand = terminalCommand;
    req.session.notebooks.terminalCommandArgs = terminalCommandArgs;
