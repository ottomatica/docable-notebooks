...
exports.get_notebook = async function (req, res) {
    ...
    // HACK for session state quick-fix. Uncomment if your environment selection is out-of-whack.
    // req.session.notebooks.setup = {'local':'local'};
...
async function _get_notebook(req, res, notebook_dir, slug, cwd) {
    try {
        const notebook = await notebookManager._get_notebook(req, res, notebook_dir, slug, cwd);

        const session = req.session;
        const provider = Object.keys(session.notebooks.setup)[0];
        const isHosted = req.originalUrl.startsWith('/examples/');
        const name = notebookSlug.slug2notebook(slug);
...
exports.addImage = async function (req, res) {
    envManager.addDockerImage({ ...req.body });
