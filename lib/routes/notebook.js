...
exports.runCell = async function(req, res) {
    ...
    if( session.notebooks )
    {
        ...
        results = results.map(res => {
            return { result: { ...res.result, stdout: res.result.stdout, stderr: res.result.stderr} , cellindex: stepIndex }
        });
...
exports.runUnsafe = async function (req, res) {
    ...
    const session = req.session;
    const $ = cheerio.load(req.body.notebook);

    if( session.notebooks )
    {  
        let { results, _, status } = await docable.fromHtml($, session.notebooks.setup, session.notebooks.cwd);
        results = results.map(res => {
            return { result: { ...res.result, stdout: res.result.stdout, stderr: res.result.stderr} , cellindex: res.cell.index }
        });
