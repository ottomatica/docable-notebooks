...
async function run(text, stepIndex, session, res)
{
    ...
    if( session.notebooks )
    {
        ...
        // making secrets obj: {secret_slug: secret_value}
        let secrets = {};
        for(const s of utils.getSecrets()) {
            secrets[s.slug] = s.secret;
        }

        docable.fromHtml($, session.notebooks.setup, session.notebooks.cwd, session.notebooks.docDir, onProgress, secrets).then(function(data)
        {
