...
async function run(text, stepIndex, session, res)
{
    ...
    if( session.notebooks )
    {
        ...
        // create container if provider == docker
        if (provider == 'docker') {
            const containerName = session.notebooks.setup.docker;
            await utils.createContainer(session, containerName, CONTAINER_TIMEOUT);
        }

        docable.fromHtml($, session.notebooks.setup, session.notebooks.cwd, session.notebooks.docDir, onProgress).then(function(data)
        {
...
exports.runHosted = async function (req, res) {
    ...
    await utils.createContainer(req.session, containerName, CONTAINER_TIMEOUT);
