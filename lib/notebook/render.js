...
class NotebookRender
{
    async notebookRender(rootDir, md, ir) {
        ...
        try{
            ...
            let baseDir = "";
            if( rootDir != ".") baseDir = rootDir + path.sep;
            $('img').each(function () {
                const link = $(this).attr('src');
                if (!link.startsWith('http') && !link.startsWith('//')) {
                    let fixedUrl = "/imgs/" + notebook2slug(baseDir) + notebook2slug(link);
            ...
            $('a').each(function () {
                ...
                if (link && link.match(/[.]md[#]?/) && !link.startsWith('http') && !link.startsWith('//')) {
                    let fixedUrl = "/notebooks/" + notebook2slug(baseDir) + notebook2slug(link);
