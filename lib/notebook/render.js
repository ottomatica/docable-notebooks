...
class NotebookRender
{
    ...
    async renderCell(elem, $, executable, sessionId)
    {
            ...
            <div class="mb-2">
                <div class="d-flex flex-column card">
                    <div>
                        ...
                        <iframe class="w-100 playground-preview" frameBorder="0" id="html_playground_preview_${elIDWithoutDash}"></iframe>
                    </div>
                </div>
        ...
        if (el.data('grade_this') && el.data('grade_this_check')) {
            if (el.data('type') != 'playground') cell.addClass('d-none');
            cell.append(`
            <div class="d-flex flex-row">
                <div class="ml-auto mr-4 my-auto check-playground-result"></div>
                <button class="btn btn-primary mt-2 check-playground-btn" onclick="$('#${el.attr('id')}').parent().find('.play-btn').trigger('click'); ">Grade this</button> 
            </div>`);
        }
