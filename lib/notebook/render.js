...
class NotebookRender
{
    async notebookRender(session, rootDir, md, ir) {
        ...
        try{
            ...
            $('[data-docable="true"]').each(function (index, elem) {
                ...
                let cell = self.renderCell($(elem), $, policy.isExecutable($(elem), session.notebooks.setup), session.id );
    ...
    async renderCell(elem, $, executable, sessionId)
    {
        ...
        if (!['quiz', 'file', 'script'].includes(el.data('type')) && (!isHosted || process.env.DOCABLE_CONTENTEDITABLE))
        ...
        if( executable )
        {
            ...
            if (el.data('type') === 'repl')
            {
                ...
                el.append(`<script> 
                    ...
                    socket${elIDWithoutDash}.emit('connect to room', window.location.pathname);
                    new TerminalUI(socket${elIDWithoutDash}, $('#${el.attr('id')}').parent('.docable-cell-overlay')[0]);
            ...
            el.after(
                `<script>
                    ...
                    require(['vs/editor/editor.main'], function () {
                        ...
                        m_${elIDWithoutDash} = monaco.editor.create(el_${elIDWithoutDash}[0], {
                        ...
                        m_${elIDWithoutDash}.onDidContentSizeChange(() => $('#${el.attr('id')}').height(Math.min(480, Math.max(45, m_${elIDWithoutDash}.getContentHeight()) )));
                    });
