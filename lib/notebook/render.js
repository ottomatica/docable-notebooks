...
class NotebookRender
{
        ...
        try{
            ...
            $('img').each(function () {
                ...
                if (!link.startsWith('http') && !link.startsWith('//')) {
                    let fixedUrl = "/imgs/" + notebookSlug.notebook2slug(path.normalize( path.join(baseDir, link)));
            ...
            $('a').each(function () {
                ...
                if (link && link.match(/[.]md[#]?/) && !link.startsWith('http') && !link.startsWith('//')) {
                    let fixedUrl = "/notebooks/" + notebookSlug.notebook2slug(path.normalize( path.join(baseDir, link)));
    ...
    async renderCell(elem, $, executable)
    {
        ...
        if( el.data("range") )
        {
            el = this._highlightRange(el, el.data("range") );
        }

        // This is now set by docable.
        // el.attr('id', uuid.v4() );
