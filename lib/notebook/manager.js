...
async function _get_notebook(req, res, notebook_dir, notebookMDPath) {
    ...
    try {
        ...
        const notebookMDSetup = await stepper.getSetupFromDocument(notebookMDPath);
        let setupStanza = (req.session.notebooks && req.session.notebooks.setup) || notebookMDSetup;
        if (setupStanza.local) setupStanza.local.cwd = notebookMDSetup.local.cwd;
