...
function getAvailableEnvironments(targets, session, notebookUrl) {
    ...
    if (!targets || targets.length < 2) {
        ...
        environments.push({
            type: 'multi',
            targets: targets.map(t => ({ ...t, name: t.name || t.type, slug: t.name || t.type })),
            id: uuid.v4()
        });
...
async function environmentStatus(currentEnvironment) {
    let targets = currentEnvironment;
    if (currentEnvironment.type.includes('multi'))
        targets = currentEnvironment.targets;
    else
        targets = [targets]
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        if (i == 0)
            targets[i].default = true;

        try {
            const conn = Connectors.getConnector(targets[i].type, targets[i].host || targets[i].name, targets[i]);
            targets[i].status = await conn.ready();
        }
        catch (err) {
...
module.exports = {
    ...
    prepareEnvironment, nukeEnvironment, environmentStatus, getAvailableEnvironments, 
    getConnectorSlug
};