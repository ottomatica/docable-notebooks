...
function getConnectorSlug(environment) {
    if (environment.type === 'multi') {
        ...
        return `multi-docker: ${environment.map(env => env.name).join(', ')}`;
    }
...
async function prepareEnvironment(session, currentEnvironment, cwd, containerTimeout) {
    // cwd = ~ if local
    // cwd = repo_dir if git
    ...
    if (currentEnvironment.type.includes('multi')) {
        for (let target of currentEnvironment.targets) {
            if (target.type == 'docker') {
                ...
                try { child_process.exec(`docker network connect ${session.id} ${target.name} --alias ${target.slug}`, () => { }) } catch (err) { }
                try { child_process.exec(`docker network prune -f`) } catch (err) { }

                await createContainer(target.image, target.name, containerTimeout);
            }
        }
    }
    ...
    else if (currentEnvironment.type === 'ssh') {
        session.notebooks.cwd = '.'
    }
}
